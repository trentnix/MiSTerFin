...
Video output goes through the standard MiSTer framebuffer path (`mplayer -vo fbdev:/dev/fb0`) and works on any menu core — MiSTerFin's patched mplayer waits for vsync before each frame write, so playback is tear-free there in normal use. If the [Zaparoo Project](https://zaparoo.org)'s dual-mode menu core (`menu_zaparoo.rbf`) happens to be installed as your menu core, MiSTerFin detects it and switches playback to its DDR native path instead: double-buffered page flips (tear-free by construction rather than by timing) with a genuine broadcast-style scanout. That's a trade-off, not an upgrade — the native path scans out at a lower horizontal resolution than the scaler path, and side by side the two are hard to tell apart. No configuration either way; on the stock core the native path simply stays off.
