...
/* 8x8 bitmap font covering ASCII 0x00-0x7F.
   Each character is 8 bytes (one per row).
   Bit 0 of each byte = leftmost pixel (LSB-first order).
   Source: dhepper/font8x8 (public domain), based on IBM VGA font via Marcel Sondaar. */
...
/* Latin-1 Supplement U+00A0-U+00FF (accented Latin etc.) — same
   dhepper/font8x8 public-domain source as font8x8_basic above. Index i
   is code point 0x00A0+i. Lets the UI render Portuguese/Spanish/French/
   German accents instead of folding them to bare ASCII. */
static const uint8_t font8x8_ext_latin[96][8] = {
